...
import { generateJUnitXml } from './junit-xml';

/**
 * Default k6 handleSummary callback. Produces stdout text, HTML, JSON, and JUnit XML reports.
 * Output directory and test name are configurable via K6_SUMMARY_DIR and K6_TEST_NAME env vars.
 */
...
/**
 * Extracts a simplified summary object from raw k6 summary data.
 * Includes test duration, HTTP request stats, and custom trend metrics ending in `_ms`.
 */
export function generateSummary(data: K6SummaryData): K6Summary {
...
function generateHTMLReport(summary: K6Summary, timestamp: string): string {
    ...
    const trendsRows = summary.trends
        .map(
            (t) => `
            <tr>
                <td>${t.name}</td>
